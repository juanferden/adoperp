...
class JFormFieldFbconnect  extends JFormFieldList
{
	...
	function getInput()
	{
		...
		<script>	
			...
			function getlink(){			
				...
				if(apid.trim().length  != 0 && client_secret.trim().length !=0){ 
					link =\"https://www.facebook.com/dialog/oauth?client_id=\" + apid + \"&redirect_uri=\" + encodeURIComponent (window.location+'&tmpl=component') + \"&scope=offline_access,read_stream,publish_stream,user_groups\";
				}else{
		...
		$html .= '<input class="btnload"  type="button" value="'.JText::_("PUBLISHED_FACEBOOK_PROFILE").'" onclick="JavaScript:newPopup(getlink(),\'Connecting to Facebook ...\',500,450);"/>';
