...
/**
* Kunena Component
* @package Kunena.Template.Bowthemes
*
...
$cssurl = JURI::root(true) . '/components/com_kunena/template/bowthemes/css';
?>
<!--[if lte IE 7]>
...
$forumHeader = $template->params->get('forumHeadercolor', $skinner ? '' : '#3C3C3C');
...
$announcementHeader = $template->params->get('announcementHeadercolor', $skinner ? '' : '#3C3C3C');
...
$frontStatsHeader = $template->params->get('frontstatsHeadercolor', $skinner ? '' : '#3C3C3C');
...
$onlineHeader = $template->params->get('whoisonlineHeadercolor', $skinner ? '' : '#3C3C3C ');
...
if ($inactiveFont) {
	$styles .= <<<EOF
	#Kunena #ktab a span { color: {$inactiveFont} ; }
EOF;
}
$activeFont = $template->params->get('activeFontcolor', $skinner ? '' : '#C6234D');

if ($activeFont) {
	$styles .= <<<EOF
	#Kunena #ktab #current a span { color: {$activeFont}; }
EOF;
}
