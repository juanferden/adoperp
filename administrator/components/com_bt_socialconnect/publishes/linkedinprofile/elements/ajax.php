...
class JFormFieldAjax extends JFormFieldList {
	...
	public function prepareData($user){
		...
		$html .='<a href='.$user->publicProfileUrl.' target="_blank" >'.$user->formattedName.'</a></div>';
		$html .='<input type="hidden" value="'.$user->id.'"" name="group[id]"/>';			
		$html .='<input type="hidden" value="'.$user->formattedName.'"" name="group[name]"/><br/>';	
		$html .='<input type="hidden" value="'.$user->pictureUrl.'"" name="group[image]"/><br/>';			
		return $html;
	}
