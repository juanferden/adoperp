		...
		if ($module->showtitle != 0) {
			for($i=0; $i <= strlen($module->title); $i++){
					if(substr($module->title,$i,1)==' '){
						...
						$title = '<span>'.$str.'</span>'.' '.$str1;								
						break;
						}
					else{
						if($i==strlen($module->title)){
							$title = $module->title;
						}
					}
			}
			$html .= "<{$headerTag} class=\"module-title {$headerClass}\"><span class=\"module-title-inner\">{$title}</span></span></{$headerTag}>";
		}

		$html .= "<div class=\"module-ct\"><div class=\"module-ct-inner\">{$module->content}</div></div></div></{$moduleTag}>";

		echo $html;
	}
}

function modChrome_btjsocial($module, &$params, &$attribs)
{ 
	$badge          = preg_match ('/badge/', $params->get('moduleclass_sfx'))? '<span class="badge">&nbsp;</span>' : '';
	$moduleTag      = htmlspecialchars($params->get('module_tag', 'div'));
	$headerTag      = htmlspecialchars($params->get('header_tag', 'h3'));
	...
	if (!empty ($module->content)) {
		$html = "<{$moduleTag} class=\"jomsocial-module {$moduleClassSfx} {$moduleClass}\" id=\"Mod{$module->id}\">" .
					"<div class=\"module-inner\">" . $badge;
...
{ 
	...
	if (!empty ($module->content)) {
		$html = "<{$moduleTag} class=\"t3-module module{$moduleClassSfx} {$moduleClass}\" id=\"Mod{$module->id}\">" .
					"<div class=\"container module-inner\">" . $badge;
		...
		if ($module->showtitle != 0) {
			...
			$html .= "<{$headerTag} class=\"module-title {$headerClass}\"><span>{$title}</span></{$headerTag}>";
		}
