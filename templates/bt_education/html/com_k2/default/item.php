...
<div id="k2Container" class="itemView<?php echo ($this->item->featured) ? ' itemIsFeatured' : ''; ?><?php if($this->item->params->get('pageclass_sfx')) echo ' '.$this->item->params->get('pageclass_sfx'); ?>">
  ...
  <div class="itemToolbar">
		<ul>
			...
			<!-- Item Hits -->
			<span class="itemHits">
				<?php echo JText::_('BT_K2_READ'); ?> <?php echo $this->item->hits; ?>
			</span>
			<?php endif; ?>			
  ...
  <!-- Latest items from author -->
	<div class="item-col-of2 1">
	<div class="itemAuthorLatest">
		<?php 
			$k2LatestFrom = JText::_('K2_LATEST_FROM');
			if($k2LatestFrom != "") :
				for($i = 0; $i <= strlen($k2LatestFrom); $i++) {
					if(substr($k2LatestFrom,$i,1) == " ")  {
						$subk2LatestFrom1 = substr($k2LatestFrom,0,$i);
						$subk2LatestFrom2 = substr($k2LatestFrom,$i);
						break;
					}
				}
			endif;
		?>
		<h3><?php echo "<span>".$subk2LatestFrom1."</span>".$subk2LatestFrom2; ?> <?php echo $this->item->author->name; ?></h3>
		<ul>
  ...
  <!-- Related items by tag -->
	<div class="itemRelated">
		<?php 
			$k2Related = JText::_("K2_RELATED_ITEMS_BY_TAG");
			if($k2LatestFrom != "") :
				for($i = 0; $i <= strlen($k2Related); $i++) {
					if(substr($k2Related,$i,1) == " ")  {
						$subk2Related1 = substr($k2Related,0,$i);
						$subk2Related2 = substr($k2Related,$i);
						break;
					}
				}
			endif;
		?>	
		<h3><?php echo "<span>".$subk2Related1."</span>".$subk2Related2; ?> </h3>
		<ul>
