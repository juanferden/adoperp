	...
	function getInput()
	{
		...
		<script>	
			...
			function getlink(){			
				...
				if(apid.trim().length  != 0 && client_secret.trim().length !=0){
					link =\"https://www.linkedin.com/uas/oauth2/authorization?response_type=code&client_id=\" + apid + \"&redirect_uri=\" + encodeURIComponent (window.location+'&tmpl=component') + \"&scope=r_fullprofile+r_emailaddress+r_network+rw_nus+rw_groups+w_messages+rw_company_admin\" +\"&state=linkedingroup\";
				}else{
		...
		if(!empty($this->value)){		
			...
			foreach($this->value AS $key=>$value){	
				...
				$avatar = 'group id = '.$value->id . '::' . '<img width="100" src="' . $value->image . '" alt = "' . $value->image . '">';				
				$html .="<input type='checkbox' ".$check."   value=\"".$value->id."\" name='group[$key][checked] '/> ".
				'<span class="editlinktip hasTip" title="'.htmlspecialchars($avatar).'">'.'<a href="http://www.linkedin.com/groups/Fan-torres-5162538?home=&gid='.$value->id.'" target="_blank">'.$value->name."</a>".'</span>'.
				"<br/>";			
				$html .="<input type='hidden'    value=\"".$value->id."\" name='group[$key][id] '/><br/>";			
				$html .="<input type='hidden'   value=\"".$value->name."\" name='group[$key][name]'/>";	
				$html .="<input type='hidden' value=\"".$value->image."\" name='group[$key][image]'/>";
			}
			$html .="</div>";
		}
		$html .='<div id="groups"></div>';
		$html .= '<input class="btnload"  type="button" value="'.JText::_("PUBLISHED_LINKEDIN_GROUPS").'" onclick="JavaScript:newPopup(getlink(),\'Connecting to Facebook ...\',500,450);"/>';
