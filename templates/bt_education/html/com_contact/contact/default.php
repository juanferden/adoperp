...
<?php endif; ?>
	...
	<?php if ($this->params->get('show_email_form') && ($this->contact->email_to || $this->contact->user_id)) : ?>
		...
		<?php if ($this->params->get('presentation_style')=='plain'):?>
			<?php
				$textContactForm = JText::_('COM_CONTACT_EMAIL_FORM');
				if( $textContactForm != "") {
					for($i=0; $i <= strlen($textContactForm); $i++)
					{
						if(substr($textContactForm,$i,1) == " "){
							$subtextContactForm1 = substr($textContactForm,0,$i);
							$subtextContactForm2 = substr($textContactForm,$i);
							break;
						}
					}
				}
			?>
			<?php if(isset($subtextContactForm1) && isset($subtextContactForm2)) {
				echo '<h3><span>'. $subtextContactForm1.'</span> '.$subtextContactForm2.'</h3>'; 
			} else {
