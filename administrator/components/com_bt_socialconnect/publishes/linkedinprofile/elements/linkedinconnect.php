...
class JFormFieldLinkedinconnect  extends JFormFieldList
{
	protected $type = 'Linkedinconnect';
	...
	{		
		...
		<script>	
			...
			function getlink(){			
				...
				if(apid.trim().length  != 0 && client_secret.trim().length !=0){
					link =\"https://www.linkedin.com/uas/oauth2/authorization?response_type=code&client_id=\" + apid +\"&scope=r_fullprofile+r_emailaddress+r_network+rw_nus+w_messages+rw_groups+rw_company_admin\"+\"&state=linkenconnect\" +\"&redirect_uri=\" + encodeURIComponent (window.location+'&tmpl=component');
				}else{
		...
		if(!empty($this->value)){		
			...
			$html .='<div style="height:50px;"><a href="http://www.linkedin.com/profile/view?id='.$data->uid.'" target="_blank"><img style="width:50px; height:50px;float:left;margin:5px" src="'.$data->groupid->image.'"></a><br/>';			
			$html .='<a href="http://www.linkedin.com/profile/view?id='.$data->uid.'" target="_blank">'.$data->uname.'</a></div>';
			$html .='<br/>';			
			
			$html .="</div>";
		}
		$html .='<div id="groups"></div>';
		$html .= '<input class="btnload"  type="button" value="'.JText::_("PUBLISHED_LINKEDIN_PROFILE").'" onclick="JavaScript:newPopup(getlink(),\'Connecting to Linkedin ...\',500,450);"/>';
